...
- (UILabel *)dateLabel {
    if(!dateLabel && pullToRefreshActionHandler) {
        ...
        dateLabel.font = [UIFont fontWithName:AUVBoldFont size:12];
...
- (void)setPullToRefreshActionHandler:(void (^)(void))actionHandler {
    ...
    titleLabel.font = [UIFont fontWithName:AUVBoldFont size:18];
    ...
    if(pullToRefreshActionHandler) {
        switch (newState) {
            ...
            case SVPullToRefreshStateLoading:
                titleLabel.text = NSLocalizedString(@"Loading. Please wait...",);
