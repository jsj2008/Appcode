...
//	}
    ...
    AFHTTPRequestOperation *conection=[[AFHTTPRequestOperation alloc] initWithRequest:request];
    [conection setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *op,id responseObject)
    {
    
        
        NSError* error;
        if(self.logging == YES) {
            NSString* response = [[NSString alloc] initWithData: conection.responseData encoding: NSUTF8StringEncoding];
            NSLog(@"%@", response);
        }
        
        
        CXMLDocument* doc = [[CXMLDocument alloc] initWithData: conection.responseData options: 0 error: &error];
        if(doc == nil) {
            [self handleError:error];
            return;
        }
        ...
        if([fault hasFault]) {
            ...
            CXMLNode* element = [[Soap getNode: [doc rootElement] withName: @"SOAP-ENV:Body"] childAtIndex:0];
