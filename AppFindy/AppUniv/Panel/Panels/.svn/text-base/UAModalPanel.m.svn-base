...
#define DEFAULT_BACKGROUND_COLOR	[UIColor colorWithWhite:1.0 alpha:0.9]
#define DEFAULT_CORNER_RADIUS		4.0f
#define DEFAULT_BORDER_WIDTH		5.0f
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self != nil) {
		...
		[self setBackgroundColor:[UIColor colorWithWhite:0.0 alpha:0.1]]; // Fixed value, the bacground mask.
...
- (UIButton*)actionButton {
	if (!actionButton) {
		...
		[self.actionButton setBackgroundImage:image2 forState:UIControlStateHighlighted];
		...
		self.actionButton.titleLabel.font = [UIFont fontWithName:AUVBoldFont size:11];
...
- (CGRect)closeButtonFrame {
 ...
 return CGRectMake(f.origin.x - floor(closeButton.frame.size.width*0.5),f.origin.y - floor(closeButton.frame.size.height*0.5),closeButton.frame.size.width,closeButton.frame.size.height);
}
